...
export function CotisationForm({
  ...
  return (
    <form action={formAction} className="space-y-3 rounded-2xl bg-white p-4 shadow-[0_2px_10px_rgba(28,28,23,.08)]">
      <h2 className="font-semibold text-[#1C1C17]">Enregistrer un versement</h2>
      <p className="text-sm text-[#6B6B60]">Saisissez le montant reçu : il s&apos;impute automatiquement sur le retard du membre.</p>
      {state?.erreur && (
      ...
      <div className="flex flex-wrap gap-2">
        <input name="montant" type="number" step="0.01" min="0.01" defaultValue={montantDefaut} required className="w-28 rounded-lg border border-[#E2DFD6] p-2" />
